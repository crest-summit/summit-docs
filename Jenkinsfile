node {
    ...
    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'aws-jenkins-credentials', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
        sh "docker run --rm -v $location:/aws mikesir87/aws-cli jq '.family=\"docs-dev\" | .containerDefinitions[0].image=\"606752838354.dkr.ecr.us-east-1.amazonaws.com/summit/docs:build-2\"' task-definition.json > updated-task-definition.json"
