node {
    ...
    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'aws-jenkins-credentials', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
        ...
        sh "docker run --rm -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID} -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY} -v $location:/aws mikesir87/aws-cli aws ecs register-task-definition --cli-input-json file://updated-task-definition.json"
        sh "docker run --rm -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID} -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY} -v $location:/aws mikesir87/aws-cli aws ecs update-service --cluster static-dev --service docs-dev-service --task-definition docs-dev"
